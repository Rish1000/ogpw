...
class AnomalyDetector:
    ...
    def _get_default_config(self) -> Dict:
        return {
            ...
            "suspicious_ips": {"1.2.3.4", "100.200.10.20"},
    ...
    def _detect_latency_spikes(self):
        ...
        for packet in self.packets:
            if 'dns' in packet['layers']:
                ...
                if 'query_name' in dns_layer:
                    dns_queries[tx_id] = packet
                elif 'answers' in dns_layer and tx_id in dns_queries:
                    query_packet = dns_queries[tx_id]
                    rtt = (packet['timestamp'] - query_packet['timestamp']) * 1000
    ...
    def _detect_suspicious_endpoints(self):
        ...
        for packet in self.packets:
            if 'ip' in packet['layers']:
                src_ip = packet['layers']['ip']['src']
                dst_ip = packet['layers']['ip']['dst']
...
async def detect_anomalies(pcap_json_data: List[Dict]) -> Dict:
    detector = AnomalyDetector(parsed_packets=pcap_json_data)
    results = detector.run_detection()
    return {
        "summary": f"{len(results)} anomalies detected.",
        "anomalies": results
    }
